...
module('Challenges | javascript.arrays', function (hooks) {
  ...
  module('lastN', function () {
    ...
    test('n < array.length returns last n elements', function (assert) {
      assert.deepEqual(
        lastN([1, 2, 3, 4, 5], 2),
        [4, 5],
        'input array length 5, n=2 returns last two elements'
      );
    });
    ...
    test('n > array.length returns entire array', function (assert) {
      assert.deepEqual(
        lastN([1, 2, 3, 4, 5], 9),
        [1, 2, 3, 4, 5],
        'input array length 5, n=9 returns full array'
      );
    });
  });
  ...
  module('negatorator', function () {
    ...
    test('handles 0s and non-numbers gracefully', function (assert) {
      assert.deepEqual(
        negatorator([0, 'string', '1.23']),
        [0, NaN, -1.23],
        'basic positive array is negated'
      );
    });
  });
  ...
  module('sanitizer', function () {
    ...
    test('correctly removes yucky values', function (assert) {
      ...
      assert.deepEqual(
        sanitizer([[1], [], { a: 1 }, {}, 'bass', '']),
        [[1], { a: 1 }, 'bass'],
        'empty arrays/objects/strings are removed'
      );
      assert.deepEqual(sanitizer([12, 13, 14, '13']), [12, 14, '13'], 'the number 13 is removed');
      assert.deepEqual(
        sanitizer(['the', 'very', 'moist', 'cake', 'was', 'tasty', 'and', 'MOIST']).join(' '),
        'the very cake was tasty and',
        'the word "moist" is removed'
      );
      assert.deepEqual(
        sanitizer(['a', 'c', 'q', 'quark', 'Q', 'quack', 'ABQ']),
        ['a', 'c', 'quark', 'quack', 'ABQ'],
        'the letter "q" is removed'
      );
    });
  });
  ...
  module('annoyingCalculator', function () {
    test('empty inputs return empty array', function (assert) {
      ...
      assert.deepEqual(
        annoyingCalculator(undefined),
        [],
        'undefined array input returns empty array'
      );
  ...
  module('mergeBySize', function () {
    ...
    test('properly merges arrays', function (assert) {
      ...
      assert.deepEqual(
        mergeBySize([[3, 3, 3], [2, 2], [1]]),
        [3, 3, 3, 2, 2, 1],
        'arrays already sorted'
      );
      assert.deepEqual(
        // prettier-ignore
        mergeBySize([['k'], ['f', 'g', 'h'], ['i', 'j'], ['l'], ['a', 'b', 'c', 'd', 'e']]).join(''),
        'abcdefghijkl',
        'array variety'
      );
    });

    test('properly merges strings', function (assert) {
      assert.deepEqual(
        mergeBySize(['k', 'fgh', 'ij', 'l', 'abcde']),
