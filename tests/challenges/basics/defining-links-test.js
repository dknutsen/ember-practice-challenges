...
module('Challenges | basics.defining-links', function(hooks) {
  ...
  test('the route link is defined correctly', async function(assert) {
    ...
    assert.dom('#route-link a').hasAttribute('href', '/basics/defining-links?foo=bar&id=basics.defining-links', 'the link href is correct');
    assert.dom('#route-link a').hasText('Route link', 'the link text is correct');
    await click('#route-link a');
    assert.equal(currentRouteName(), 'basics.defining-links', 'the route is correct after clicking the link');
